...
from .models import (
    ...
    Cat,
    Payment,
)  # <-- Client должен быть импортирован
...
def checkout(request):
    ...
    try:
        ...
        order = Order.objects.get(
            client_id=request.session["user_id"], status="pending"
        )
    except Order.DoesNotExist:
        return redirect("menu:menu")

    errors = {"error_cat": None, "error_payment": None, "error_order": None}
...
def menu_list(request):
    ...
    if request.method == "POST" and "add_to_order" in request.POST:
        ...
        try:
            ...
            order_item, created = OrderItem.objects.get_or_create(
                order=order, menu_item=menu_item, defaults={"quantity": quantity}
            )
            if not created:
    ...
    return render(
        request, "menu/menu.html", {"categories": categories, "current_order": order}
    )
